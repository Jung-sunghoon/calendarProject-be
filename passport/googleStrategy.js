  ...
  passport.use(
    ...
    new GoogleStrategy(
      ...
      async (accessToken, refreshToken, profile, done) => {
        ...
        try {
          let rows;
          [rows] = await pool.query("SELECT * FROM users WHERE email = ?", [profile?.emails[0].value]);
          ...
          if (rows) {
            ...
            try {
              const result = await pool.query("INSERT INTO users (email, username, nickname) VALUES (?, ?, ?)", [
                newUser.email,
                newUser.username,
                newUser.nickname,
              ]);
